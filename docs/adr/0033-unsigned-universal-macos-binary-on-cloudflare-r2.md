# ADR-0033 — An unsigned universal macOS binary, hosted on Cloudflare R2
...
builds with `--target universal-apple-darwin`, so a single artifact carries both
the arm64 and the x86_64 slice, packages it as
`datagraph-<version>-macos-universal.tar.gz` — tar rather than zip, because it
...
- the **versioned** key, `datagraph/datagraph-<version>-macos-universal.tar.gz`,
  which the Homebrew formula pins (ADR-0034) so an old formula keeps installing
  the version it was written for;
- the **stable** key, `datagraph/datagraph-macos.tar.gz`, which is what the
...
**The binary is signed ad hoc, not by Apple.** `lipo` — which is what
`--target universal-apple-darwin` runs to fuse the two slices — invalidates the
arm64 slice's signature, and macOS will not execute an arm64 binary whose
signature is broken. `codesign --force -s -` restores a valid signature with no
identity behind it, which is all that requirement asks for.
...
- **Two per-arch binaries.** Cheaper to build, more expensive everywhere else:
  two assets, two URLs, and a user who has to know which Mac they own. The
  universal build costs a few minutes, spent once per release.
...
- **The stable-key URL is load-bearing.** `datagraph/datagraph-macos.tar.gz` is
...
- **Releases exist for macOS only.** Linux and Windows stay build-from-source,
  and the README says so rather than implying a download that isn't there.
