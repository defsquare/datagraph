...
describe("incremental relayout", () => {
  ...
  it("keeps median drift on already-placed cards at (near) zero, at scale", async () => {
    // Scénario réaliste d'un expand isolé : 167 paires client/commande
    // indépendantes (bigShop(3000)), les 167 clients visibles et posés, une
    // seule commande révélée par l'expand d'un seul agrégat. C'est le chiffre
    // qui justifie l'épinglage : voir la section "Graph view" des READMEs.
    ...
    const customerIds = [...graph.nodes.values()]
      .filter((n) => n.kind === "entity" && n.entityType === "Customer")
      .map((n) => n.id)
    const collapsed = new Set(customerIds)
    const before = await engine.layout(graph, aggregates, collapsed)

    const expanded = new Set([...collapsed, "/orders/0"])
    const after = await engine.layoutAfterExpand(before, graph, aggregates, "Customer#c0", expanded)

    const drifts = [...before.positions]
      .map(([id, rect]) => {
        const moved = after.positions.get(id)!
        return Math.hypot(moved.x - rect.x, moved.y - rect.y)
      })
      .sort((a, b) => a - b)
    const mid = Math.floor(drifts.length / 2)
    const median = drifts.length % 2 === 0 ? (drifts[mid - 1]! + drifts[mid]!) / 2 : drifts[mid]!

    // Mesuré : n=167, median≈0px, max=55.62px (le pinning fige la
    // quasi-totalité des nœuds exactement ; le pire cas vient de
    // `separateOverlaps`, qui n'a pas de notion de "pinned" et peut encore
    // écarter un nœud épinglé mordu par la carte nouvellement insérée — c'est
    // le même mécanisme que la tolérance de 40px du test au-dessus, à une
    // autre échelle). La médiane elle-même n'est pas toujours *exactement* 0 :
    // exécutée seule, `toBe(0)` passe, mais dans la suite complète elle a
    // mesuré ~1.1e-12px — du bruit de calcul flottant sur les positions
    // pinnées, pas un vrai déplacement. D'où l'épsilon, 6 ordres de grandeur
    // sous le pixel et donc sans rapport avec les 40px tolérés pour un nœud.
    expect(median).toBeLessThanOrEqual(1e-6)
    expect(drifts.length).toBe(customerIds.length)
  })
